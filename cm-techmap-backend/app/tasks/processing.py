...
def process_drone_upload(
    ...
    try:
        ...
        if odm_healthy and len(image_files) >= 3:
            ...
            for _ in range(max_polls):
                ...
                if odm_status == "completed":
                    logger.info("  ODM processing completed!")
                    break
            ...
            if odm_log:
                logger.info("[PIPELINE] ODM processing log (last 50 lines):")
        ...
        if dsm_raw_path:
            from app.core.cog_converter import convert_to_cog, extract_elevation_stats, extract_geospatial_metadata
            ...
            try:
                ...
                if model_files.is_valid():
                    ...
                    with open(model_offset_path, "w") as f:
                        ...
                        f.write("# Generated by CM TECHMAP Model Converter\n")
...
def generate_report(self, project_id: str, report_type: str, parameters: dict):
    ...
    try:
        ...
        # Save report record to DB (canonical columns; file_key = "bucket/object"
        # so the API can split it when generating presigned download URLs)
        engine2 = create_engine(settings.database_url_sync, isolation_level="AUTOCOMMIT")
        try:
            with engine2.connect() as conn:
                conn.execute(sa_text("""
                    INSERT INTO public.reports
                        (project_id, report_type, title, status, file_key,
                         output_format, file_size_bytes, config)
                    VALUES
                        (:pid, :type, :title, 'completed', :key,
                         :fmt, :size, :params)
                """), {
                    "pid": project_id,
                    "type": report_type,
                    "title": report_config.get("title", "Relatório"),
                    "key": f"{settings.minio_bucket_reports}/{report_key}",
                    "fmt": ext,
